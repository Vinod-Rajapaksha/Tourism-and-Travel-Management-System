        ...
        try {
            ...
            if (errorMessage != null &&
                    (errorMessage.contains("UQ__Guides__AB6E6164") ||
                            ...
                            (errorMessage.contains("duplicate key") && errorMessage.toLowerCase().contains("email")) ||
                            (errorMessage.contains("Duplicate entry") && errorMessage.toLowerCase().contains("email")))) {
            ...
            if (errorMessage != null &&
                    (errorMessage.contains("UQ__Guides__") && errorMessage.toLowerCase().contains("nic")) ||
                    (errorMessage.contains("duplicate key") && errorMessage.toLowerCase().contains("nic")) ||
                    (errorMessage.contains("Duplicate entry") && errorMessage.toLowerCase().contains("nic"))) {
                return ResponseEntity.status(HttpStatus.CONFLICT)
                        .body(Map.of(
                                "error", "NIC already exists. Please use a different NIC.",
                                "field", "nic"
                        ));
            }
